...
class UNet3DConditionModel(ModelMixin, ConfigMixin):
    ...
    @classmethod
    def from_pretrained_2d(cls, pretrained_model_path, subfolder=None, unet_additional_kwargs=None, device="cpu", torch_dtype=torch.float16):
        ...
        safetensors_path = glob.glob(model_file.replace(".bin", "*.safetensors"))
        print(safetensors_path)
        if len(safetensors_path) > 0:
            state_dict = {}
            with safe_open(safetensors_path[0], framework="pt", device=device) as f:
                for key in f.keys():
